...
class LocationControllerTest {
  ...
  fun `getLocationsByDeviceWearerId should return bad request when it does not receive a valid id`() {
    ...
    val result = assertThrows<EmApiError> { LocationController(locationService).getLocationsByDeviceWearerId(deviceWearerId) }
  ...
  fun `getLocationsByDeviceWearerIdAndTimeFrame should return bad request when it does not receive valid deviceWearerId`() {
    ...
    val result = assertThrows<EmApiError> { LocationController(locationService).getLocationsByDeviceWearerIdAndTimeFrame(deviceWearerId, startDate, endDate) }

    confirmException(result, expected)
    verify(locationService, times(0)).getAllLocationsByDeviceWearerIdAndTimeFrame(any<String>(), any<Date>(), any<Date>())
  }
  ...
  fun `getLocationsByDeviceWearerIdAndTimeFrame should return internal server error when there is an internal server issue`() {
    ...
    assertThrows<Exception> { LocationController(locationService).getLocationsByDeviceWearerIdAndTimeFrame(deviceWearerId, startDate, endDate) }
    verify(locationService, times(1)).getAllLocationsByDeviceWearerIdAndTimeFrame(any<String>(), any<Date>(), any<Date>())
  }
